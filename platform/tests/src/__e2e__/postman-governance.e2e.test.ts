...
describe("Postman / governance (1:1 generated)", () => {
  ...
  it("listCarbonIntensityRecords", async () => {
    const url = sub("{{baseUrl}}/v1/governance/carbon-intensity?period={{period}}");
  ...
  it("listGovernanceEvents", async () => {
    const url = sub("{{baseUrl}}/v1/governance/events?cursor={{cursor}}&limit={{limit}}&eventType={{eventType}}");
    const res = await fetch(url, {
      method: "GET",
      headers: vars.accessToken ? { Authorization: `Bearer ${vars.accessToken}` } : {},
    });
