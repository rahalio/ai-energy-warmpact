 ...
 *
 * API client for underwriting domain.
...
const rawUnderwritingService = {
  /**
   * Underwrite Building Connection
   */
  async getUnderwriting(connectionId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/connections/${connectionId}/underwriting`;
   ...
   */
  async getUnderwriting(connectionId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/connections/${connectionId}/underwriting` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   */
  async getThermalModel(connectionId: string, params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/connections/${connectionId}/thermal-model` + (params ? `?${new URLSearchParams(params).toString()}` : '');
...
export const underwritingService = makeService(rawUnderwritingService, "underwriting");
