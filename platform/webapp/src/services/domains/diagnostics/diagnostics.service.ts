 ...
 * Diagnostics Service
 *
 * API client for diagnostics domain.
...
const rawDiagnosticsService = {
  /**
   * List Fault Findings
   */
  async getFaultFinding(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/diagnostics/fault-findings` + (params ? `?${new URLSearchParams(params).toString()}` : '');
   ...
   * Dispatch Work Order
   */
  async getWorkOrder(findingId: string, data?: any, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/diagnostics/fault-findings/${findingId}/work-order`;
...
export const diagnosticsService = makeService(rawDiagnosticsService, "diagnostics");
