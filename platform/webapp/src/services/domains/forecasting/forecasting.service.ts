 ...
 * Forecasting Service
 *
 * API client for forecasting domain.
...
const rawForecastingService = {
  /**
   * Get Demand Forecast
   */
  async getDemand(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/forecasts/demand` + (params ? `?${new URLSearchParams(params).toString()}` : '');
...
export const forecastingService = makeService(rawForecastingService, "forecasting");
