...
const rawGovernanceService = {
   ...
   */
  async getCarbonIntensity(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    const orgId = getEffectiveOrgId();
    if (!orgId) {
      throw new Error("Organization ID is required");
    }

    const url = `/orgs/${orgId}//v1/governance/carbon-intensity` + (params ? `?${new URLSearchParams(params).toString()}` : '');
  ...
  async getEvent(params?: Record<string, any>, signal?: AbortSignal): Promise<any> {
    ...
    const url = `/orgs/${orgId}//v1/governance/events` + (params ? `?${new URLSearchParams(params).toString()}` : '');
