...
export function getOrCreateTenantProfile(tenantId: string): SandboxTenantProfile {
  ...
  const profile: SandboxTenantProfile = {
    tenantId,
    displayNameEn: tenantId === 'tnt_demo' ? 'Warmpact Demo Tenant' : tenantId,
    displayNameAr: tenantId === 'tnt_demo' ? 'وارمباكت' : undefined,
    status: 'active',
    createdAt: nowIso(),
  };
...
export function generateApiKeySecret(prefix = 'warmpact_demo'): string {
